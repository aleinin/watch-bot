...
const reactNTimes = (n) =>
  Array.from(Array(n))
    .reduce((accumulator) => [...accumulator, nestArray('✅', '❌', '❓')], [])
    .flat(1)

const nestArray = (...args) => args.map((arg) => [arg])
...
describe('Reactions', () => {
  ...
  beforeEach(() => {
    ...
    mockSend.mockImplementation((message) =>
      Promise.resolve(sentMessage(message))
    )
  })
  ...
  describe('handleQuestionReactions', () => {
    it('send question, then choices and react to them', async () => {
      ...
      expect(mockSend.mock.calls).toEqual(
        nestArray('question?', 'choice! one!', 'choice two', '!@#$', 'd')
      )
